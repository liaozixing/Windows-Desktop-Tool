...
from PyQt5.QtWidgets import QFileDialog, QHBoxLayout, QStackedWidget, QVBoxLayout, QWidget
        ...
        if is_svg:
            if target_fmt == "ICO":
                ...
                save_path, _ = QFileDialog.getSaveFileName(
                    self, f"保存 {target_fmt}", f"output.{target_fmt.lower()}", f"{target_fmt} 图片 (*.{target_fmt.lower()})"
                )
                if save_path:
                    try:
