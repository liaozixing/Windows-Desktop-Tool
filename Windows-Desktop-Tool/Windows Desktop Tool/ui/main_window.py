...
# 导入工具模块
from ui.dialogs.disclaimer_dialog import DisclaimerDialog, DISCLAIMER_TEXT

# 应用版本号（直接在此处修改）
APP_VERSION = "v1.3.0"

# 更新日志内容（直接在此处修改）
CHANGELOG_TEXT = """v1.3.0 (2026-01-25)
...
4. [优化] 版本号管理：版本号和更新日志直接写入代码，更新更方便。
5. [安全] 免责声明升级：强制阅读倒计时与代码内置声明文本，提升合规性。
6. [调整] IP 查询优化：取消启动自动查询，改为手动触发，保护用户隐私。
7. [优化] 系统兼容性：图标显示修复，优先使用 .ico 格式图标，提供更好的视觉体验。"""
from ui.workers.background_workers import IPWorker, SpeedTestWorker, GPFixWorker
...
class MainWindow(FluentWindow):
    ...
    def init_window(self):
        ...
        if hasattr(sys, '_MEIPASS'):
            ...
            # 开发环境：获取 main_window.py 所在目录的父目录（Windows Desktop Tool）
    ...
    def init_tray(self):
        ...
        if hasattr(self, 'app_icon') and not self.app_icon.isNull():
            ...
            # 如果图标文件不存在，QSystemTrayIcon 会使用默认图标，但会显示警告
    ...
    def quit_app(self):
        """优化后的退出方法，快速释放资源"""
        # 停止所有定时器
        if hasattr(self, '_speed_chart_timer'):
            self._speed_chart_timer.stop()
        ...
        # 停止所有工作线程（设置超时，避免卡死）
        workers = ['speed_worker', 'ip_worker', 'gp_worker']
        ...
        if hasattr(self, 'window_tool_interface'):
            try:
                if hasattr(self.window_tool_interface, 'highlighter'):
                    self.window_tool_interface.highlighter.close()
                if hasattr(self.window_tool_interface, 'target_btn'):
